...
| 2026-08-25→26 | **Full six-head sweep.** Junk-negative retrain: gated numbers unchanged, gate structural. Derived composition (min/product) loses. Brandon's [[methods/stacked-composition-mlp\|stacked composition MLP]] (OOF stacking) closes duplex/triplet: 15.52/9.73 vs baseline 11.32/7.54 — hybrid beats the baseline on all six label types. YOLO26x 23.30 (candidate-depth caveat). Proposal skeleton drafted: [[directions/thesis-proposal-fall-2026\|fall-2026 proposal]] |
| 2026-08-26→27 | **Exp12 default run complete** ([[findings/exp12-phrase-head-attribution\|five attributed findings]]): phrase mechanism validated (+1.4 duplex / +1.1 triplet on aligned features, 41/49 rare triplets), encoder is the bottleneck, alignment gates the mechanism, combination ties the record — features are the one remaining lever. Thesis-best action 16.07 |
| next | Moradi: phrase review (Amber near-synonym fix), proposal defense date; September timebox: true-clip re-cache then 1B encoder; writing in parallel |
